...
| 2 | **Private repository.** | **Launch.** | The repo is public. A scan found nothing that must not be public: the Supabase JWT is the **anon** key (`"role":"anon"`) and RLS is the real boundary; no `service_role` key, no private key, no API secret. So this is hygiene, not an exposed credential — **and it is not a free action.** The site is served by GitHub Pages from `main` with a `CNAME`, and **Pages from a private repo requires a paid GitHub plan**. On a free org plan, flipping visibility takes the live site down. Confirm the plan first, or move hosting, then flip. |
| 3 | **Supabase backup / PITR check.** | **Real customer data.** | Confirm point-in-time recovery is enabled and a restore has actually been **rehearsed** — not just that backups appear in the dashboard. **PITR covers Postgres only.** Uploaded plan PDFs live in Supabase **Storage** (`PLANS_BUCKET`), which PITR does not restore; it needs its own backup. A restore that brings back every `projects` row and no plan PDFs is a half-restore. |
...
| 8 | **Stripe is in test mode.** `stripeInstance` is constructed from a hard-coded `pk_test_…` publishable key, with no live/test swap. | **Taking real money.** | A publishable key is safe to expose — this is not a leak. It means checkout runs against Stripe **test** mode, so a real customer’s card would never be charged and no payment would reach the account. Swap to the live publishable key, and confirm the server side is on live secrets too. |
